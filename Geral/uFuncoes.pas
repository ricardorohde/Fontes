...
function GeraArquivoTaraBalancaMGV6(consulta : TUniquery): string;
Function VerificaSeVendeuNoCartao(iVenda: Integer): Boolean;
...
begin
  ...
  begin
    ...
    begin
      margem_lucro := roundto(((valor_venda / (custo_total)) - 1)* 100, -2);
...
begin
  ...
  begin
    ...
    begin
      ...
      linha := linha + AcertaTexto(consulta.FieldByName('tar_001').AsString, 'D', 4, '0'); //codigo tara 4 posiçoes
...
begin
    ...
    begin
      ...
      begin
        ...
        InserirMovimentoProduto(item_composicao,qtd_composicao,tipo_mov,observacao,
                                ...
                                0, 0, data_movimento, false, false, id_setor_movimento );
        qr_aux.Next;
      end;
    end;
    FreeAndNil(qr_aux);
  ...
  begin
    qrcombo := CriaUniQuery(nil, frmMenu.conexao);

    qrcombo.Close;
    qrcombo.SQL.Text:= 'select * from materiais_combo where id_material = :id_material and id_empresa = :emp';
    qrcombo.ParamByName('id_material').AsInteger:= id_produto;
    qrcombo.ParamByName('emp').AsInteger:= recproj.iEmp;
    qrcombo.Open;

    qrcombo.First;
    while not qrcombo.Eof do
    begin
      tipo_mov := UpperCase(tipo_mov);
      qr_aux := CriaUniQuery(nil, frmMenu.conexao);
      str_sql := 'select b_atualiza_custo_material_composicao, b_considera_rendimento_entrada_composicao from empresas where emp_001='+inttostr(recproj.iEmp);
      ...
      PreparaQuerySQL(qr_aux, str_sql);
      qr_aux.ParamByName('id_empresa').AsInteger   := RecProj.iEmp;
      qr_aux.ParamByName('id_produto').AsInteger   := qrcombo.FieldByName('id_produto_combo').AsInteger;
      qr_aux.ParamByName('quantidade').AsFloat     := quantidade * qrcombo.FieldByName('quantidade').AsFloat;
      qr_aux.ParamByName('id_usuario').AsInteger   := id_usuario;
      qr_aux.ParamByName('observacao').AsString    := observacao;
      qr_aux.ParamByName('tipo_mov').AsString      := tipo_mov;
      qr_aux.ParamByName('data').AsDateTime        := data_movimento;
      ...
      begin
        ...
        //tenta atualizar, se não houver resgistro de setor de estoque, insere
        if ExecutaComandoSQL(str_sql, vararrayof([quantidade * qrcombo.FieldByName('quantidade').AsFloat, qrcombo.FieldByName('id_produto_combo').AsInteger, id_setor_movimento, recproj.iEmp]))<1 then
        begin
          str_sql := ' insert into setor_estoque_material (id_material, id_setor, id_empresa, quantidade) '+
                     ' values (:id_material, :id_setor, :id_empresa, (0.0 ' + sinal_mov + ' :quantidade)) ';
          ExecutaComandoSQL(str_sql, vararrayof([qrcombo.FieldByName('id_produto_combo').AsInteger, id_setor_movimento, recproj.iEmp, quantidade * qrcombo.FieldByName('quantidade').AsFloat]));
        end;
      ...
      begin
        ...
        { Cáclculo exemplo
          ...
          str_sql := format('select rendimento from composicao where id_composicao=%d and id_empresa=%d',[qrcombo.FieldByName('id_produto_combo').AsInteger, recproj.iEmp]); ;
          BuscaCampo(rendimento_composicao, str_sql, '', false);
        ...
        begin
          str_sql:=' update composicao set valor_custo=:valor_custo where id_composicao=:id_produto and id_empresa=:id_empresa';
          ExecutaComandoSQL(str_sql, vararrayof([valor_custo, qrcombo.FieldByName('id_produto_combo').AsInteger, RecProj.iEmp]));
        end;
      ...
      if atualiza_custo_pela_composicao and (uppercase(tipo_item_mov)='C') and (tipo_mov='E') then
        AtualizaPrecoCustoMaterialPelaComposicao(qrcombo.FieldByName('id_produto_combo').AsInteger);

      //atualiza a margem de lucro com base nos novos valores
      if (tipo_mov = 'E') and (uppercase(tipo_item_mov)='M') and (id_venda<=0) then AtualizaMargemMaterial(qrcombo.FieldByName('id_produto_combo').AsInteger);
      ...
      begin
        str_sql :=format(' select mc.quantidade, mc.id_composicao, c.id_setor from  materiais_composicao mc ' +
                  ' join composicao c on c.id_composicao=mc.id_composicao and c.id_empresa = mc.id_empresa ' +
                  ' where mc.id_material = %d and mc.id_empresa = %d', [qrcombo.FieldByName('id_produto_combo').AsInteger, RecProj.iEmp]);
        ExecutaConsultaSQL(qr_aux, str_sql);
        while not qr_aux.Eof do
        begin
          qtd_composicao := (quantidade * qrcombo.FieldByName('quantidade').AsFloat) * qr_aux.Fieldbyname('quantidade').AsFloat ; //Quantidade do material * a quantidade do item de composicao
          item_composicao := qr_aux.Fieldbyname('id_composicao').AsInteger ;
          id_setor_movimento := qr_aux.Fieldbyname('id_setor').AsInteger;
...
begin
  ...
  begin
    ...
    if VerificaSeVendeuNoCartao(id_venda) then
    begin
      Application.CreateForm(TFGerenciaTef, FGerenciaTef);
      FGerenciaTef.ShowModal;
      FreeAndNil(FGerenciaTef);
    end;
    ...
    begin
      ...
      begin
        InserirMovimentoProduto(qrAux1.FieldByName('id_material').AsInteger,
                                ...
                                'M', true,0, 0, 0, date+time, false, false, qrAux1.FieldByName('id_setor').AsInteger);
...
begin
  ...
  begin
    ...
    begin
      //formato da linha
      //CCCC(4)TTTTTT(6)D(20)
      linha := AcertaTexto(consulta.FieldByName('tar_001').AsString, 'D', 4, '0'); //codigo
      int_aux := round(consulta.FieldByName('peso').asfloat * 1000);
      linha := linha + AcertaTexto(IntToStr(int_aux), 'D', 6, '0'); //peso deve ser inteiro com tres casas exemplo: 2,350-> 2350
      linha := linha + AcertaTexto(consulta.FieldByName('descricao').asstring, 'E', 20, ' ') ; //descricao 20 posições
      linha := linha + quebra;
      saida := saida + linha;
      consulta.Next;
    end;
...
begin
   ...
   str_sql := format('select count(e.for_001) as qtde from encerravenda e, formapgto f where e.for_001 = f.for_001 and f.b_tef = ''T'' '+
                     ' and e.emp_001=%d and e.ven_001=%d', [RecProj.iEmp, iVenda]) ;
   ExecutaConsultaSQL(qrAux, str_sql);
